...
namespace :area_forecast_system do
  desc "Complete area forecast loca M in data/*.csv"
  task :complete do
    puts "area forecast local M complete"
    version = "20240216"
    area_forecast_locals = JMACode::AreaForecastLocalM.load(version: version)
    area_relation = JMACode::AreaForecastLocalM.load_relation(version: version)
    ...
    CSV.open(path, "wb", row_sep: JMACode::AreaForecastLocalM::CSV_ROW_SEP) do |csv|
      (JMACode::AreaForecastLocalM::NUM_HEADER_ROWS - 1).times.each do
        csv << []
      end
      csv << JMACode::AreaForecastLocalM::HEADERS
      area_forecast_locals.each do |local|
